...
class Pokemons {
    ...
    public static void selection() { // COMPARAÇÕES: n*10 / Como sabemos quee serão 10 movimentações a complexidade
                                     // passa a ser "O(n)" visto que 10 será um número constante a partir desse
                                     // momento
        for (int i = 0; i < 10; i++) { // COMPARAÇÕES: 10 movimentações
            comparisons++;
            int menor = i;
            for (int j = (i + 1); j < arrayPokemos.size(); j++) { // COMPARAÇÕES: "n" movimentações
                comparisons++;
                if (arrayPokemos.get(menor).getName().compareTo(arrayPokemos.get(j).getName()) > 0) {
                    menor = j;
                }
            }
            movements++;
            Collections.swap(arrayPokemos, menor, i);
        }
    }
    ...
    public static void generatorLog(long startTime, long endTime) {
        ...
        try {
            ...
            writer.write(matricula + "\t" + executionTime + "\t" + "O(n*10)" + "\t" + movements + "\t" + comparisons);
            writer.close();
        } catch (IOException e) {
