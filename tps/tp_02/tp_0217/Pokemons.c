...
int compareDates(Date date_01, Date date_02)
{

    if (date_01.year < date_02.year)
    {
        return -1;
    }
    else if (date_01.year > date_02.year)
    {
        return 1;
    }

    if (date_01.month < date_02.month)
    {
        return -1;
    }
    else if (date_01.month > date_02.month)
    {
        return 1;
    }

    if (date_01.day < date_02.day)
    {
        return -1;
    }
    else if (date_01.day > date_02.day)
    {
        return 1;
    }
}
...
void swap(Pokemons *a, Pokemons *b)
{
    ...
    *a = *b;
    *b = temp;
    movements++; // Incrementa a movimentação ao trocar
}

int getMaiorFilho(Pokemons *array, int i, int tamHeap)
{
    int filho;
    comparisons++; // Comparando filhos
    comparisons++;
    if (2 * i == tamHeap)
    {
        filho = 2 * i;
    }
    else if (array[2 * i].height > array[2 * i + 1].height)
    {
        filho = 2 * i;
    }
    else if (array[2 * i].height < array[2 * i + 1].height)
    ...
    {
        comparisons++;
        if (strcmp(array[2 * i].name, array[2 * i + 1].name) > 0)
        {
            filho = 2 * i;
        }
        else
...
void construir(Pokemons *array, int tamHeap)
{
    for (int i = tamHeap; i > 1; i /= 2)
    {
        comparisons++; // Comparação do tamanho do heap
        if (array[i].height > array[i / 2].height ||
            (array[i].height == array[i / 2].height && strcmp(array[i].name, array[i / 2].name) > 0))
        {
...
void reconstruir(Pokemons *array, int tamHeap)
{
    int i = 1;
    while (i <= tamHeap / 2)
    {
        comparisons++; // Comparação se i está dentro do limite
        int filho = getMaiorFilho(array, i, tamHeap);
        comparisons++;
        if (array[i].height < array[filho].height ||
            (array[i].height == array[filho].height && strcmp(array[i].name, array[filho].name) < 0))
        {
...
void heapsort(Pokemons *array, int n)
{
    // Alterar o vetor ignorando a posição zero
    Pokemons arrayTmp[n + 1];
    for (int i = 0; i < n; i++)
    {
    ...
    while (tamHeap > 1)
    {
        comparisons++; // Comparação para verificar se tamHeap > 1
        swap(&arrayTmp[1], &arrayTmp[tamHeap--]);
        reconstruir(arrayTmp, tamHeap);
    }

    // Copiar os resultados de volta
    for (int i = 0; i < n; i++)
    {
...
{
    char matricula[] = "863593_heapsort";
    char nomeArquivo[20];
