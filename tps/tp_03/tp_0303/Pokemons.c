...
typedef struct // "Classe" Pokemons
{
    ...
    List types;     // Lista -> aponta para uma posição da stacka
    List abilities; // Lista -> aponta para uma posição da stacka
    double weight;
    double height;
    int captureRate;
    bool isLegendary;
...
// Inciar stacka
void initList(List *stack)
{
    stack->n = 0;
    for (int i = 0; i < MAX_LIST; i++)
    {
        stack->element[i] = NULL; // Inicializa os elementos como NULL
    }
}

// Insere no início da stacka
int insertList(List *stack, char *el)
{
    if (stack->n < MAX_LIST)
    {
        stack->element[stack->n] = (char *)malloc(strlen(el) + 1);
        if (stack->element[stack->n] != NULL)
        {
            strcpy(stack->element[stack->n], el);
            stack->n++;
            return 1;
        }
    }
...
// Mostra elementos da stacka
void showList(List *stack)
{
    for (int i = 0; i < stack->n; i++)
    {
        printf("'%s'", stack->element[i]);
        if (stack->element[i + 1] != NULL)
        {
            printf(", ");
        }
    }
...
void init(ListPokemons *stack)
{
    stack->n = 0;
}

void inserirFim(ListPokemons *stack, Pokemons x)
{

    // validar insercao
    if (stack->n >= MAX_LIST_CLASS)
    {
        printf("Erro ao inserir!");
        exit(1);
    }

    stack->element[stack->n] = x;
    stack->n++;
}

Pokemons removerFim(ListPokemons *stack)
{
    ...
    stack->n--;                                       // Decrementa primeiro
    Pokemons ultimoPokemon = stack->element[stack->n]; // Armazena o Pokémon removido
    return ultimoPokemon;
}

void mostrar(ListPokemons *stack)
{
    int i;
...
// Variáveis globais
static Pokemons stackPokemons[MAX_CLASS];
...
Pokemons *findID(int id)
{
    ...
    {
        if (stackPokemons[i].id == id)
        {
            return &stackPokemons[i];
        }
    }
